...
class App:
    ...
    def __call__(self, request: httpx2.Request) -> httpx2.Response:
        ...
        return httpx2.Response(self.status_code, headers=headers, json=data)
...
class DigestApp:
    ...
    def challenge_send(self, request: httpx2.Request) -> httpx2.Response:
        ...
        return httpx2.Response(401, headers=headers)


class RepeatAuth(httpx2.Auth):
    """
...
class ResponseBodyAuth(httpx2.Auth):
    """
...
class SyncOrAsyncAuth(httpx2.Auth):
    """
...
async def test_basic_auth() -> None:
    ...
    async with httpx2.AsyncClient(transport=httpx2.MockTransport(app)) as client:
...
async def test_custom_auth() -> None:
    ...
    def auth(request: httpx2.Request) -> httpx2.Request:
...
def test_netrc_auth_credentials_exist() -> None:
    ...
    app = App()
    auth = httpx2.NetRCAuth(netrc_file)

    with httpx2.Client(transport=httpx2.MockTransport(app), auth=auth) as client:
...
def test_auth_hidden_url() -> None:
    ...
    assert url == httpx2.URL(url)
    assert expected == repr(httpx2.URL(url))
...
def test_digest_auth_returns_no_auth_if_alternate_auth_scheme() -> None:
    ...
    client = httpx2.Client(transport=httpx2.MockTransport(app))
...
async def test_digest_auth_qop_must_be_auth_or_auth_int() -> None:
    ...
    async with httpx2.AsyncClient(transport=httpx2.MockTransport(app)) as client:
        with pytest.raises(httpx2.ProtocolError):
            await client.get(url, auth=auth)
...
def test_sync_digest_auth_raises_protocol_error_on_malformed_header(
    ...
    with httpx2.Client(transport=httpx2.MockTransport(app)) as client:
        with pytest.raises(httpx2.ProtocolError):
...
async def test_digest_auth_unavailable_streaming_body() -> None:
    ...
    async with httpx2.AsyncClient(transport=ConsumeBodyTransport(app)) as client:
