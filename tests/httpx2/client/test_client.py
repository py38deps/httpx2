...
def test_merge_relative_url_with_encoded_slashes() -> None:
    ...
    client = httpx2.Client(base_url="https://www.example.com/base%2Fpath")
...
def test_context_managed_transport() -> None:
    ...
    transport = Transport()
    with httpx2.Client(transport=transport):
        pass
...
def test_context_managed_transport_and_mount() -> None:
    ...
    with httpx2.Client(transport=transport, mounts={"http://www.example.org": mounted}):
        pass
...
def unmounted(request: httpx2.Request) -> httpx2.Response:
    data = {"app": "unmounted"}
    return httpx2.Response(200, json=data)


def mounted(request: httpx2.Request) -> httpx2.Response:
    data = {"app": "mounted"}
    return httpx2.Response(200, json=data)
...
def test_mounted_transport() -> None:
    ...
    client = httpx2.Client(transport=transport, mounts=mounts)
