...
async def test_http_connection() -> None:
    ...
    async with AsyncHTTPConnection(origin=origin, network_backend=network_backend, keepalive_expiry=5.0) as conn:
        ...
        async with conn.stream("GET", "https://example.com/") as response:
            assert repr(conn) == "<AsyncHTTPConnection ['https://example.com:443', HTTP/1.1, ACTIVE, Request Count: 1]>"
...
async def test_http2_connection() -> None:
    ...
    async with AsyncHTTPConnection(origin=origin, network_backend=network_backend, http2=True) as conn:
...
class NeedsRetryBackend(AsyncMockBackend):
    ...
    async def connect_tcp(
        ...
        stream = await super().connect_tcp(host, port, timeout=timeout, local_address=local_address)
        return self._NeedsRetryAsyncNetworkStream(self, stream)

    class _NeedsRetryAsyncNetworkStream(AsyncNetworkStream):
        def __init__(self, backend: NeedsRetryBackend, stream: AsyncNetworkStream) -> None:
...
async def test_connection_retries() -> None:
    ...
    async with AsyncHTTPConnection(origin=origin, network_backend=network_backend, retries=3) as conn:
        response = await conn.request("GET", "https://example.com/")
        assert response.status == 200
...
async def test_uds_connections() -> None:
    ...
    async with AsyncHTTPConnection(origin=origin, network_backend=network_backend, uds="/mock/example") as conn:
        response = await conn.request("GET", "https://example.com/")
        assert response.status == 200
